                ...
                <div className={styles.cardBody}>
                  ...
                  <div className={styles.cardLinks}>
                    ...
                    <button
                      ...
                      style={{ background: 'rgba(255,200,0,0.06)', color: 'var(--accent)', borderColor: 'rgba(255,200,0,0.2)' }}
                    >
              ...
              <div className={styles.modalLinks}>
                ...
                {selectedProject.live && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`${styles.modalLink} ${styles.modalLinkLive}`}
              ...
              <div className={styles.modalContent}>
                ...
                {selectedProject.techSpecs ? (
                  <div className={styles.specs}>
                    <h4 className={styles.specsTitle}>Technical Implementation</h4>

                    {/* Key Metrics */}
                    {selectedProject.techSpecs.metrics && (
                      <div className={styles.metricsGrid}>
                        {selectedProject.techSpecs.metrics.map((m) => (
                          <div key={m.label} className={styles.metricCard}>
                            <span className={styles.metricValue} style={{ color: selectedProject.color }}>
                              {m.value}
                            </span>
                    ...
                    {/* Backend */}
                    {selectedProject.techSpecs.backend && (
                      <div className={styles.specSection}>
                        <h5 className={styles.specSubTitle} style={{ color: selectedProject.color }}>
                          Backend Architecture (Express.js + Node.js + MongoDB)
                        </h5>
                        <ul className={styles.specList}>
                          {selectedProject.techSpecs.backend.map((item, idx) => (
                            <li key={idx}>{item}</li>
                          ))}
                        </ul>
                      </div>
                    ...
                    {/* Security */}
                    {selectedProject.techSpecs.security && (
                      <div className={styles.specSection}>
                        <h5 className={styles.specSubTitle} style={{ color: selectedProject.color }}>
                          Security & Middleware
                        </h5>
                        <ul className={styles.specList}>
                          {selectedProject.techSpecs.security.map((item, idx) => (
                            <li key={idx}>{item}</li>
                          ))}
                        </ul>
                      </div>
                    ...
                    {/* Frontend */}
                    {selectedProject.techSpecs.frontend && (
                      <div className={styles.specSection}>
                        <h5 className={styles.specSubTitle} style={{ color: selectedProject.color }}>
                          Frontend Architecture (React 18 + Vite)
                        </h5>
                        <ul className={styles.specList}>
                          {selectedProject.techSpecs.frontend.map((item, idx) => (
                            <li key={idx}>{item}</li>
                          ))}
                        </ul>
                      </div>
                    ...
                    {selectedProject.techSpecs.payments && (
                      <div className={styles.specSection}>
                        <h5 className={styles.specSubTitle} style={{ color: selectedProject.color }}>
                          Payment & Media Integration
                        </h5>
                        <ul className={styles.specList}>
                          {selectedProject.techSpecs.payments.map((item, idx) => (
                            <li key={idx}>{item}</li>
                          ))}
                        </ul>
                      </div>
                    ...
                    {/* Deployment */}
                    {selectedProject.techSpecs.deployment && (
                      <div className={styles.specSection}>
                        <h5 className={styles.specSubTitle} style={{ color: selectedProject.color }}>
                          Deployment Stack
                        </h5>
                        <ul className={styles.specList}>
                          {selectedProject.techSpecs.deployment.map((item, idx) => (
                            <li key={idx}>{item}</li>
                          ))}
                        </ul>
                      </div>
                  ...
                  // General Tech Tags if no detailed specs
                  <div className={styles.genericSpecs}>
                    <h4 className={styles.specsTitle}>Technologies Used</h4>
                    <div className={styles.techTags}>
                      {selectedProject.tech.map((t) => (
                        <span key={t} className="tag" style={{ borderColor: `${selectedProject.color}40`, color: selectedProject.color }}>
                          {t}
                        </span>
                      ))}
                    </div>
